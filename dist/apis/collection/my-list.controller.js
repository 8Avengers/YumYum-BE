...
const my_list_detail_dto_1 = require("./dto/my-list.detail.dto");
let MyListController = class MyListController {
    constructor(myListService) {
        this.myListService = myListService;
    }
    async getMyListsDetail(collectionId, currentUser) {
        const myLists = await this.myListService.getMyListsDetail(currentUser.id, collectionId);
        return await myLists;
    }
    async getMyListsDetailPost(restaurantId, data, currentUser) {
        const myLists = await this.myListService.getMyListsDetailPost(currentUser.id, restaurantId, data.collectionId);
        return await myLists;
    }
...
__decorate([
    (0, common_1.Get)('/collections/datail/:collectionId'),
...
], MyListController.prototype, "getMyListsDetail", null);
__decorate([
    (0, common_1.Get)('/collections/posts/:restaurantId'),
    (0, decorators_1.UseGuards)(auth_guards_1.AuthAccessGuard),
    (0, swagger_1.ApiOperation)({ summary: 'MyList 전체조회(내꺼)' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'MyList 전체조회(내꺼) 성공' }),
    (0, swagger_1.ApiResponse)({ status: 400, description: 'MyList 전체조회(내꺼) 실패' }),
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, my_list_detail_dto_1.DetailMylistDto, Object]),
    __metadata("design:returntype", Promise)
